...
CREATE POLICY IF NOT EXISTS projects_select_by_token ON projects
  FOR SELECT
...
CREATE POLICY IF NOT EXISTS subcontractors_select_public ON subcontractors
  FOR SELECT
  USING (
    EXISTS (
...
CREATE POLICY IF NOT EXISTS documents_select_public ON documents
  FOR SELECT
  USING (
    EXISTS (
      SELECT 1 FROM projects 
...
CREATE POLICY IF NOT EXISTS documents_insert_public ON documents
  FOR INSERT
  WITH CHECK (
    EXISTS (
      SELECT 1 FROM projects 
